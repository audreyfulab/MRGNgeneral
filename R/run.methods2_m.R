...
run.methods2 <- function (simdata = NULL,
  ...
  # Initialize whitelist0 and blacklist0 as NULL (will be populated later if applicable)
  whitelist0 <- NULL
  blacklist0 <- NULL
  ...
  if(any(c('tabu', 'hc', 'pc.stable', 'mmhc', 'fast.iamb', 'mmpc', 'hpc') %in% bn.methods)) {
    if (!is.null(conf.sets)) {
      ...
      # Save whitelist and blacklist if savetopath is TRUE
      if (savetopath) {
        save(whitelist0, file = paste0(path, setpath, "/", "whitelist0", ".RData"))
        save(blacklist0, file = paste0(path, setpath, "/", "blacklist0", ".RData"))
      }
  ...
  # Output list - MODIFIED TO INCLUDE whitelist0 and blacklist0
  out <- list(Performance = c(ConfSelection$recall,
              ...
              whitelist0 = whitelist0,  # ADDED
              blacklist0 = blacklist0,  # ADDED
