...
#'
#' @param old.Adj Adjacency matrix used to form trios in \code{trio.set}.
#'
#' @param Adj Updated adjacency matrix after the analysis of trios in \code{trio.set}.
#'
#' @param q,p,r Numbers of respectively \code{V,T,Q}-nodes in \code{Adj}. I.e.,
#' the first \code{q} columns/rows of \code{Adj} correspond to \code{V}-nodes,
#' the next \code{p} columns/rows correspond to \code{T}-nodes, and the last
#' \code{r} columns/rows correspond to \code{Q}-nodes.
#'
...
enumerate.new.trios <- function (trio.set,   # n x 3 matrix of n already analysed trios (in which new edges were added or not)
                                 ...
                                 Adj,        # new adjacency matrix
                                 q, p,       # numbers of V-nodes and T-nodes, respectively
                                 r = NROW(Adj) - p - q, # Number of Q-nodes
                                 VTT = TRUE, Qtrio.set = NULL,
                                 cl,
                                 chunk.size = NULL) {
  # No new trio generated if no new edge OR (VTT = FALSE but r = 0)
  if (!any(new.edges) | (!VTT[1] & r == 0)) {
...
enumerate.new.trios.i <- function(col.indices,
  ...
  if (Aijk[1,2] - old.Aijk[1,2] > 0) { # When V --> T1 is the new edge
    if (VTT) { # If trios without Q-node are desired
      ...
      Qlabelk <- (Adj[col.indices[2], Qlabels] +
                    Adj[Qlabels, col.indices[2]]) > 0 # Q nodes linked to T1
    ...
    res <- rbind(res, if (length(Tlabelk)) {
      ...
      Qlabelk <- (Adj[col.indices[3], Qlabels] +
                    Adj[Qlabels, col.indices[3]]) > 0 # Q nodes linked to T1
      Qlabelk <- Qlabels[Qlabelk]                     # Replace logicals by labels
      res <- if (length(Qlabelk)) {
